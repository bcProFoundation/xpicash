...
class AbstractBlock {
  ...
  readHead(br) {
    ...
    const nonceBuf = br.readU64();
    if (nonceBuf <= 0x1fffffffffffff) {
      this.nonce = new BN(nonceBuf);
    } else {
      const data = Array.prototype.slice.call(nonceBuf, 0, 8);
      this.nonce = new BN(data, 10, 'le');
    }
    const sizeBuf = br.readU56();
    this.size = BN.fromBuffer(sizeBuf, { endian: 'little'});
