...
class TX {
  ...
  txid(enc) {
    ...
    if (!id) {
      ...
      bw.writeU32(this.verision);
      const inputHashes = this._getTxInputHashes();
      const outputHashes = this._getTxOutputHashes();
      const [inputMkerkleRoot, inputMerkleHeight] = this._getTxMerkleRootAndHeight(inputHashes);
      const [outputMkerkleRoot, outputMerkleHeight] = this._getTxMerkleRootAndHeight(outputHashes);
      bw.writeHash(inputMkerkleRoot);
      bw.writeU8(inputMerkleHeight);
      bw.writeHash(outputMkerkleRoot);
      bw.writeU8(outputMerkleHeight);
      bw.writeU32(this.locktime);
      const buf = bw.render();
      id = hash256.digest(buf);
      if (!this.mutable) {
        this._id = id;
      }
    }
  ...
  /**
   * Calculate the input hashes
   * which is used to create txid
   */
   _getTxInputHashes() {
    const hashes = [];
    if (this.inputs.length === 0) {
      return [Buffer.from(consensus.NULL_HASH, 'hex')];
    }

    for (let i = 0; i < this.inputs.length; i++) {
      const input = this.inputs[i];
      const bw = bio.pool(40);
      input.prevout.toWriter(bw);
      bw.writeU32(input.sequence);
      const hash = hash256.digest(bw.render());
      hashes.push(hash);
    }
   ...
   */
   _getTxOutputHashes() {
    ...
    for (let i = 0; i < this.outputs.length; i++) {
      ...
      const bw = bio.pool(output.getSize());
      output.toWriter(bw);
      const hash = hash256.digest(bw.render());
      hashes.push(hash);
    }
  ...
  /**
   * Calculate the merkle root and merkle tree height from the array of hash
   * Only use for lotus coin
   * Cannot use for other coins
   * @param {Array<Hash|Buffer>} hashes 
   */
  _getTxMerkleRootAndHeight(hashes) {
    if (hashes.length === 0) {
    ...
    while (size > 1) {
      ...
      for (let j = 0; j < size; j += 2) {
        const k = j + 1;
        const left = leaves[i + j];
        const right = leaves[i + k];
  
        const hash = hash256.root(left, right);
    ...
    return [hashes[hashes.length - 1], height];
  }
