...
class Headers extends AbstractBlock {
  ...
  static fromEntry(entry) {
    ...
    headers.reserved = entry.reserved;
    headers.nonce = entry.nonce;
    headers.version = entry.version;
    headers.size = entry.size;
    headers.height = entry.height;
    headers.epochBlock = entry.epochBlock;
    headers.merkleRoot = entry.merkleRoot;
    headers.extendedMetadata = entry.extendedMetadata;
    headers.metadata = entry.metadata;
  ...
  getJSON(network, view, height) {
    return {
      ...
      height: this.height,
      epochBlock: util.revHex(this.epochBlock),
      merkleRoot: util.revHex(this.merkleRoot),
      extendedMetadata: util.revHex(this.extendedMetadata),
    };
  }
